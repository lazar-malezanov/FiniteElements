...
{
    internal class AssignConcentratedLoadGlobalZDirection : AssignCommand
    {
        private IFrameLoadFactory factory;

        public AssignConcentratedLoadGlobalZDirection(IDatabase dbctx, IFrameLoadFactory factory) : base(dbctx)
        {
        ...
        public override string Execute(IList<string> parameters)
        {
            ...
            {
                throw new ArgumentException("Failed to parse AssignConcentratedLoadGlobalZDirection command parameters.");
            }
            ...
            Vector<double> EntryLoad = Vector<double>.Build.SparseOfArray(new double[] { 0, 0, loadValue });
            ...
            return $"Concentrated frame load in global Z direction with intensity {loadValue} and Load Case Numeber {loadCaseNumber} has been assigned to element with ID {frameElement.Number}.";
        }
    }
}
