...
{
    internal class AssignDistributedLoadGlobalZDirection : AssignCommand
    {
        private IFrameLoadFactory factory;

        public AssignDistributedLoadGlobalZDirection(IDatabase dbctx, IFrameLoadFactory factory) : base(dbctx)
        {
        ...
        public override string Execute(IList<string> parameters)
        {
            ...
            {
                throw new ArgumentException("Failed to parse AssignDistributedLoadGlobalZDirection command parameters.");
            }
            ...
            return $"Distributed frame load in global Z direction with intensities {loadAtStart} and {loadAtEnd} and Load Case Numeber {loadCaseNumber} has been assigned to element with ID {frameElement.Number}.";
        }
    }
}
