...
{
    internal class AssignDistributedLoadGlobalYDirection : AssignCommand
    {
        private IFrameLoadFactory factory;

        public AssignDistributedLoadGlobalYDirection(IDatabase dbctx, IFrameLoadFactory factory) : base(dbctx)
        {
        ...
        public override string Execute(IList<string> parameters)
        {
            ...
            {
                throw new ArgumentException("Failed to parse AssignDistributedLoadGlobalYDirection command parameters.");
            }
            ...
            return $"Distributed frame load in global Y direction with intensities {loadAtStart} and {loadAtEnd} and Load Case Numeber {loadCaseNumber} has been assigned to element with ID {frameElement.Number}.";
        }
    }
}
