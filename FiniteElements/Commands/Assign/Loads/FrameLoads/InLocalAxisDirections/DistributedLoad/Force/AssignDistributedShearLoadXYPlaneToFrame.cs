...
{
    internal class AssignDistributedShearLoadXYPlaneToFrame : AssignCommand
    {
        private IFrameLoadFactory factory;

        public AssignDistributedShearLoadXYPlaneToFrame(IDatabase dbctx, IFrameLoadFactory factory) : base(dbctx)
        {
        ...
        public override string Execute(IList<string> parameters)
        {
            ...
            {
                throw new ArgumentException("Failed to parse AssignDistributedShearLoadXYPlaneToFrame command parameters.");
            }
            ...
            IFrameLoad frameLoad = this.factory.CreateDistributedShearLoadXYPlane(loadCase, frameElement.ElementLength, gCoefficient,
                loadAtStart, loadAtEnd, loadStartsAt, loadEndsAt);

            FrameService.AddLoad(frameElement, frameLoad);

            return $"Distributed shear frame load in XY plane with intensities {loadAtStart} and {loadAtEnd} and Load Case Numeber {loadCaseNumber} has been assigned to element with ID {frameElement.Number}.";
        }
    }
}
