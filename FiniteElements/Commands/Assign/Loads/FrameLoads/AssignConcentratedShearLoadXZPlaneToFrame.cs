...
{
    internal class AssignConcentratedShearLoadXZPlaneToFrame : AssignCommand
    {
        private IFrameLoadFactory factory;

        public AssignConcentratedShearLoadXZPlaneToFrame(IDatabase dbctx, IFrameLoadFactory factory) : base(dbctx)
        {
        ...
        public override string Execute(IList<string> parameters)
        {
            ...
            {
                throw new ArgumentException("Failed to parse AssignConcentratedShearLoadXZPlaneToFrame command parameters.");
            }
            ...
            IFrameLoad frameLoad = this.factory.CreateConcentratedShearLoadXZPlane(loadCase, frameElement.ElementLength, gCoefficient,
                loadValue, loadPosition);

            FrameService.AddLoad(frameElement, frameLoad);

            return $"Concentrated shear frame load in XZ plane with intensity {loadValue} and Load Case Numeber {loadCaseNumber} has been assigned to element with ID {frameElement.Number}.";
        }
    }
}
